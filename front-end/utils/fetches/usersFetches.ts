...
export const createUser = async (userData: RegistrationFieldsUsed) => {
  ...
  const res = await fetch(
    `${process.env.SERVER}${ROUTES.api.users.createUser}`,
    {
      method: "POST",
      headers: {
...
export const authenticateUser = async (userData: AuthenticationFields) => {
  const res = await fetch(
    `${process.env.SERVER}${ROUTES.api.users.authenticateUser}`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
...
export const checkExistence = async (email: string) => {
  const res = await fetch(
    `${process.env.SERVER}${
      ROUTES.api.users.checkExistence
...
export const sendPasswordResetMail = async (userData: ResetPasswordFields) => {
  const res = await fetch(
    `${process.env.SERVER}${ROUTES.api.users.sendPasswordResetMail}`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
